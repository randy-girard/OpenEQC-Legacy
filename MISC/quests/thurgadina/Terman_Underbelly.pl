...
# Kiladiveus - NPC ID 115147 (lvl38) 115215(lvl40) -- Terman_Underbelly, need to change his primary faction to "coldain" 


sub EVENT_SAY { 
  if ($faction <= 3) { # Require Warmly and greater faction    
	if($text=~/hail/i){ 
		quest::emote("Terman Underbelly speaks in a rumbling tone.");
	...
	if($text=~/armor/i){
		quest::say("For you I shall make a coif. tunic. sleeves. a bracer. gauntlets. leggings. and boots.  Each piece is a quest unto itself.  To seek the components. you must search the lands for them and test the power of the spirit within.");
	}
	if($text=~/coif/i){
...
sub EVENT_ITEM { 
  if ($faction <= 3) { # Require warmly and greater faction 
	if (plugin::check_handin(\%itemcount, 25805 => 3, 24935 => 1)) { # tunic 
		...
		quest::emote("Terman smiles warmly as he hands your reward.");
