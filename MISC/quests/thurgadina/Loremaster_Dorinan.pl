...
# Kiladiveus - NPC ID 115015 - Lorekeeper_Dorinan, need to change his primary faction to "coldain" 
...
sub EVENT_ITEM { 
  if ($faction <= 3) { # Require warmly and greater faction 
	if (plugin::check_handin(\%itemcount, 25805 => 3, 24928 => 1)) { # tunic 
		...
		quest::emote("Dorinan smiles warmly as he hands your reward.");
	...
	elsif (plugin::check_handin(\%itemcount, 25827 => 3, 24929 => 1)) { # leggings 
	...
	elsif (plugin::check_handin(\%itemcount, 25840 => 3, 24934 => 1)) { # gauntlets 
