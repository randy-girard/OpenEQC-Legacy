...
sub EVENT_SAY { 
  if ($faction >= 3) { # Require Warmly and greater faction    
   if($text=~/hail/i){ 
      quest::emote("Pearce Icefang speaks in a rumbling tone."); 
   ...
   if($text=~/armor/i){ 
      quest::say("For you I shall make a cap. tunic. sleeves. a bracer. gloves. leggings. and boots.  Each piece is a quest unto itself.  To seek the components. you must search the lands for them and test the power of the spirit within."); 
   } 
   if($text=~/cap/i){ 
