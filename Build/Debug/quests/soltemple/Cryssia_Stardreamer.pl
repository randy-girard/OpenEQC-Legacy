...
sub EVENT_SAY {
  ...
  if($text=~/helm/i) {
    quest::say("In order to craft the helm, I will be needing a Mudwater Rune, an Cold Iron Helm, and a Lambent Ruby. Give these items to me and I will craft your helm");
  }
  if($text=~/vambraces/i) {
...
sub EVENT_ITEM {
  ...
  #helm
  if($itemcount{10559} && $itemcount{1508} && $itemcount{10118} == 1) {
    quest::say("Well done. Here is your Lambent Helm");
    quest::summonitem(4153);
  }

  #vambraces
  if($itemcount{4113} && $itemcount{10562} && $itemcount{10118} == 1) {
