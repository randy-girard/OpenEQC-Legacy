...
# Kiladiveus - NPC ID 115114  - Captain_Njall, need to change his primary faction to "coldain" 


sub EVENT_SAY { 
  if ($faction <= 3) { # Require Warmly and greater faction    
	if($text=~/Hail/i){
		quest::say("Well met $race! I am Captain Njall, commander of the city gaurds here in Thurgadin. If ye seek ta join our ranks, I welcome ye with open arms and have an offer to make if you are interested. If ye seek to be our enemy, Then I hope ye can run swifter than my archers arrows.");
	}
	if($text=~/i am interested/i){
		quest::say("Aye, I thought you might be. My elite warriors are in sore need of some enchanted armor. Unfortunately, I can’t spare the men to go out and fetch me the components. If you can do this for me, I’ll gladly reward you with a piece of it. I need materials for a Helm, a Breastplate, armplates, bracers, gauntlets, greaves, and boots.");
	}
	if($text=~/helm/i){
...
sub EVENT_ITEM { 
  if ($faction <= 3) { # Require warmly and greater faction 
	if (plugin::check_handin(\%itemcount, 25814 => 3, 24928 => 1)) { # tunic 
		...
		quest::emote("Njall smiles warmly as he hands your reward.");
